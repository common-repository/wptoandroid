...
class WTA_REST_Posts_Aux_WTA_Controller extends WTA_REST_Aux_WTA_Controller {
	...
	public function wta_register_routes() {
		...
		register_rest_route( $this->namespace, '/' . $this->rest_base . '/forums', array(
			'methods'         => WP_REST_Server::READABLE,
			'callback'        => array( $this, 'get_forums' ),
			'schema' => array( $this, 'get_public_item_schema' ),
		));
		register_rest_route( $this->namespace, '/' . $this->rest_base . '/topics', array(
			'methods'         => WP_REST_Server::READABLE,
			'callback'        => array( $this, 'get_topics' ),
			'schema' => array( $this, 'get_public_item_schema' ),
		));
		register_rest_route( $this->namespace, '/' . $this->rest_base . '/topics/reply', array(
			'methods'         => WP_REST_Server::READABLE,
			'callback'        => array( $this, 'get_topics_reply' ),
			'schema' => array( $this, 'get_public_item_schema' ),
		));
		register_rest_route( $this->namespace, '/' . $this->rest_base . '/questions', array(
			'methods'         => WP_REST_Server::READABLE,
			'callback'        => array( $this, 'get_questions' ),
			'schema' => array( $this, 'get_public_item_schema' ),
		));
		register_rest_route( $this->namespace, '/' . $this->rest_base . '/answers', array(
			'methods'         => WP_REST_Server::READABLE,
			'callback'        => array( $this, 'get_answers' ),
			'schema' => array( $this, 'get_public_item_schema' ),
		));
		register_rest_route( $this->namespace, '/' . $this->rest_base . '/galleries', array(
			'methods'         => WP_REST_Server::READABLE,
			'callback'        => array( $this, 'get_galleries' ),
			'schema' => array( $this, 'get_public_item_schema' ),
		));
		register_rest_route( $this->namespace, '/' . $this->rest_base . '/galleries/images', array(
			'methods'         => WP_REST_Server::READABLE,
			'callback'        => array( $this, 'get_galleries_images' ),
			'schema' => array( $this, 'get_public_item_schema' ),
		));
...
//get_topics_reply
	...
	*
	**/
	public function get_topics_reply( $request ) {
		$args                         = array();
...
/** 
	...
	* Get Galleries
	**/
	public function get_galleries_images( $request ) {
		$args                         = array();
		$args['post_parent']          = $request['ID'];
	...
	**/
	public function get_galleries( $request ) {
		...
		$querystr = "SELECT $wpdb->posts.* 
		    FROM $wpdb->posts
		    WHERE $wpdb->posts.post_content like '[gallery %' 
		    AND $wpdb->posts.post_status = 'publish' 
		    AND $wpdb->posts.post_type = 'post'";

		if (isset($args['ID'])) {
			$querystr .= " AND $wpdb->posts.ID = ".$args['ID']." ";
		}
		...
		foreach ( $query_result as $post ) {
			...
			$images = get_post_galleries_images($post->ID);
			$post->images = $images;
			$data = $this->prepare_item_for_response( $post, $request );
			$posts[] = $this->prepare_response_for_collection( $data );
		}
...
/**
	...
	**/
	public function get_topics( $request ) {
		...
		if (!isset($query_args['posts_per_page']) && $query_args['posts_per_page'] == 0 ) $query_args['posts_per_page'] = 1;
		$max_pages = ceil( $total_posts / (int) $query_args['posts_per_page'] );
	...
	public function get_items( $request ) {
		...
		if (isset($args['cat'])) {
			...
			$query = "SELECT * FROM $wpdb->posts
					...
					AND $wpdb->term_taxonomy.term_id = ".$args['cat']." ORDER BY post_date DESC";
		} else { 

			$query = "SELECT *
		        FROM  $wpdb->posts
		        WHERE $wpdb->posts.post_content NOT LIKE '[gallery %'
		        AND   $wpdb->posts.post_type = 'post'";
				
				if ($request['search'] != null && $request['search'] != '') {
					$query .= " AND ($wpdb->posts.post_title like '%".$args['s']."%' OR $wpdb->posts.post_content like '%".$args['s']."%')";
				}
				$query .= " ORDER BY $wpdb->posts.ID";
		}

		if ($args['posts_per_page'] != 0 && $args['posts_per_page'] != "") {
			if ($args['paged'] != "") $args['paged'] = $args['paged'] - 1 ;

			$minor = (int) ($args['paged'] * $args['posts_per_page']);

			$major = $minor + $args['posts_per_page'];
			$query .= " LIMIT $minor, $major";
		}
		...
		foreach ( $query_result as $post ) {
			...
			foreach ($categories as $cat) {
				$cat_var = get_cat_name($cat);
				if ($cat_var != "Uncategorized") {
					$cat_name .= $cat_var.",";
				}
			}

			$post->categories_name = $cat_name;

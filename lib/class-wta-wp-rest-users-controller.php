 ...
 * Access users
 */
class WTA_REST_Users_Aux_WTA_Controller extends WTA_REST_Aux_WTA_Controller {
...
/**
	 ...
	 */
	public function get_item_login( $request ) {
		error_reporting(E_ERROR | E_PARSE);
		$username =(string) $request['username'];
		$password =(string) $request['password'];
		$gmid = (string) $request['GM_ID'];
//GM_ID
		 $creds = array(
	        'user_login'    => $username,
        ...
        $gcm_id = get_user_meta( $user->data->ID, 'wp_to_android_gcm_regid', '' );
        if ($gcm_id == '') {
                add_user_meta( $user->data->ID, 'wp_to_android_gcm_regid', $gmid);
        }
        $user = get_user_by('id', $user->data->ID);
		$user = $this->prepare_item_for_response( $user->data, $request );
        return $user->data;//$response = rest_ensure_response( $user ); 
	}
...
//GM_ID
		$user_name = (string) $request['username'];
		...
		add_user_meta( $user->ID, 'wp_to_android_gcm_regid', $gmid);
	...
	register_rest_route( $this->namespace, '/' . $this->rest_base . '/search', array(
		...
		register_rest_route( $this->namespace, '/' . $this->rest_base . '/buddyprofile', array(
			array(
				'methods'         => WP_REST_Server::READABLE,
				'callback'        => array( $this, 'get_item_buddy_profile' ),
				'args'            => array(
					'context'     => $this->get_context_param( array( 'default' => 'view' ) ),
				),
			),
		...
		register_rest_route( $this->namespace, '/' . $this->rest_base . '/activities', array(
			array(
				'methods'         => WP_REST_Server::READABLE,
				'callback'        => array( $this, 'activities' ),
				'args'            => array(
					'context'     => $this->get_context_param( array( 'default' => 'view' ) ),
				),
			),
		...
		register_rest_route( $this->namespace, '/' . $this->rest_base . '/friends', array(
			array(
				'methods'         => WP_REST_Server::READABLE,
		...
		register_rest_route( $this->namespace, '/' . $this->rest_base . '/allowregister', array(
			array(
				'methods'         => WP_REST_Server::READABLE,
				'callback'        => array( $this, 'allow_user_register' ),
				'args'            => array(
					'context'     => $this->get_context_param( array( 'default' => 'view' ) ),
				),
			),
		...
		register_rest_route( $this->namespace, '/' . $this->rest_base . '/login', array(
				'methods'         => WP_REST_Server::EDITABLE,			
				'callback'        => array( $this, 'get_item_login' ),
				'args'            => array('test' => 'sii'),
				'schema' => array( $this, 'get_public_item_schema' ),
				));
	...
	public function get_search( $request ) {
		...
		$per_page = (int) $request['number'];
		$page = ceil( ( ( (int) $request['offset'] ) / $per_page ) + 1 );
	...
	/**
	* Check if the users can be registered in the site
	*
	**/
	function allow_user_register( $request ) {
		$data = array();
	...
	/**
	* Get all the friends from a member of buddypress
	*
	**/
	public function friends( $request ) { 
		...
		if (function_exists('bp_is_active')) {	
		    ...
		    if (!$user_id) {
		     	return new WP_Error( 'rest_user_invalid_id', __( 'You must install and activate BuddyPress plugin to use this method OR pass the correct parameters.' ), array( 'status' => 404 ) );
			}
	        ...
	        foreach ($aFriends as $sFriendID) {
				...
				$aTemp->avatar = "http:".str_replace("/", "", $aTemp->avatar) ;
	...
	/**
	*	Get buddtoress profile.
	**/
	public function get_item_buddy_profile( $request ) { 
		global $json_api;
        $oReturn = new stdClass();
		...
		if (function_exists('bp_is_active')) {	
			...
			$user_id = $request['user_id'];
			if (!$request['user_id']) {
				return new WP_Error( 'rest_user_invalid_id', __( 'Incomplete request.' ), array( 'status' => 404 ) );
			}

	        if (!bp_has_profile(array('user_id' => $user_id))) {
	            return $this->error('xprofile', 0);
	            //return new WP_Error( 'rest_user_invalid_id', __( 'User does not have BuddPress profile.' ), array( 'status' => 404 ) );
	        }
	        $oReturn = '';
	        error_reporting(E_ERROR | E_PARSE);
	        while (bp_profile_groups(array('user_id' => $user_id))) {
	            ...
	            if (bp_profile_group_has_fields()) {
	                ...
	                while (bp_profile_fields()) {
	                    ...
	                    if (trim($sFieldValue) != '' && trim($sFieldName) != '' && trim($sGroupName) != '') {
	    	...
	    	return new WP_Error( 'rest_user_invalid_id', __( 'You must install and activate BuddyPress plugin to use this method.' ), array( 'status' => 404 ) );
		}
	}

	/**
	* Function that returns the activity of the buddypress plugin of a member
	*
	**/
	public function activities( $request ) {
		...
		if (function_exists('bp_is_active')) {
	        ...
	        $aParams ['filter'] = array( 'user_id' => $user_id);
...
//				   	$aTemp->can_delete = pim_bp_activity_can_delete($user_id, (int)$aTemp->activity_id);
            	...
            	return $activities = array ('activities' => array());
            	return new WP_Error( 'rest_user_invalid_id', __( 'No activities founded' ), array( 'status' => 404 ) );
		    }

 ...
 * Access comments
 */
class WTA_REST_Comments_Aux_WTA_Controller extends WTA_REST_Aux_WTA_Controller {
	...
	public function wta_register_routes() {
		...
		register_rest_route( $this->namespace, '/' . $this->rest_base . "/allow", array(
			array(
				'methods'   => WP_REST_Server::READABLE,
				'callback'  => array( $this, 'get_comment_open' ),
				'permission_callback' => array( $this, 'get_items_permissions_check' ),
				'args'      => $this->get_collection_params(),
			),
		) );
	...
	/**
	* Function that will check if the comments can be done
	*
	**/
	function get_comment_open( $request ) {
		...
		$data = comments_open($post->ID);

	  	//$data = $this->prepare_item_for_response( $flag );
        if ($data == true) {
        	$post->commentsTotal = $post->comment_count;
        	$response = rest_ensure_response( array('id' => $post->ID) );
        } else {
            $response = rest_ensure_response( array("success" => $data) );
        }
	...
	public function create_item( $request ) {
		...
		$prepared_comment['comment_content'] = $request['comment_content'];
		$prepared_comment['comment_post_ID'] = $request['comment_post_ID'];
		$prepared_comment['comment_author'] = $request['comment_author'];
		$prepared_comment['comment_author_email'] = $request['comment_author_email'];		
// Setting remaining values before wp_insert_comment so we can
	...
	protected function prepare_links( $comment ) {
		...
		if ( 0 !== (int) $comment->user_id ) {
			$links['author'] = array(
				'href'       => rest_url( '/wp-android/users/' . $comment->user_id ),
				'embeddable' => true,
			);
		}
		...
		if ( 0 !== (int) $comment->comment_post_ID ) {
			...
			if ( ! empty( $post->ID ) ) {
				...
				$links['up'] = array(
					'href'       => rest_url( '/wp-android/' . $base . '/' . $comment->comment_post_ID ),
					'embeddable' => true,
	...
	protected function check_read_post_permission( $post ) {
		$posts_controller = new WP_REST_Posts_Aux_WTA_Controller( $post->post_type );
